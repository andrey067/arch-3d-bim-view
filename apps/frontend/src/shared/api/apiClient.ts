/**
 * Tiny `fetch` wrapper that injects a bearer token and a
 * correlation id, and offers a 401-retry hook. The actual refresh
 * implementation lands in T021 (Sprint 1, VS-Auth); for now the
 * `onUnauthorized` callback is invoked and the caller decides.
 */
...
const request = async <T>(method: string, path: string, init: ApiInit = {}): Promise<T> => {
  let response = await perform(method, path, init);
  if (response.status === 401 && onUnauthorized) {
    await onUnauthorized();
    if (authToken) {
      response = await perform(method, path, init);
    }
  }
  if (!response.ok) {
