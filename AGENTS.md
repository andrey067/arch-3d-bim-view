...
**Active scope (2026-06-10)**: IFC/SketchUp (DAE/OBJ) → Web 3D + AR MVP — simplified flow (no auth), local `/data` storage, single converter sidecar (IfcOpenShell + Blender + `usd_from_gltf`), direct file serving without redirects, `thumbnail.webp`. No direct `.skp` upload — SketchUp users export Collada. See plan.md.

Supporting artifacts (read in this order when implementing a task):
- `specs/001-ifc-mvp-platform/spec.md` — WHAT/WHY (clarifications Session 2026-06-10)
- `specs/001-ifc-mvp-platform/research.md` — technology decisions (R-11…R-21)
...
Active feature: `001-ifc-mvp-platform` (Arch3DAR — IFC/SketchUp-to-AR 3D Sharing MVP).
